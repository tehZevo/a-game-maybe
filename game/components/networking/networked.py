...
class Networked(Component):
  ...
  def for_all_networking(self, fn):
    for component in self.entity.components.values():
      if not isinstance(component, C.Networking):
        continue
      fn(component)
  ...
  def start_server(self):
    ...
    self.for_all_networking(lambda c: c.start_server())
  ...
  def get_spawn_components(self):
    ...
    spawn_components = [e.__class__.__name__ for e in self.entity.components.values() if isinstance(e, C.Networking)]

from game.networking.events import BuffsUpdated, ClientBuff
...
class BuffsSyncing(Component, NetworkBehavior, BuffsListener):
  ...
  def to_client_buff(self, buff):
    return ClientBuff(buff.id, buff.power, buff.initial_time, buff.time)
  ...
  def on_buffs_changed(self, buffs):
    ...
    buffs = [ClientBuff(b.buffdef.id, b.power, b.initial_time, b.time) for b in buffs.values()]
    evt = BuffsUpdated(networking.id, buffs)
